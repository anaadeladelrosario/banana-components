...
export const Input: React.FC<InputProps> = ({
  ...
  const renderInput = () => {
    switch (type) {
      ...
      case "RADIO":
        return (
            <RadioButton options={radioOptions} name={name} selectedValue={selectedRadioButton} onChange={(e: { target: { value: string; }; })=>handleChange(e.target.value, "RADIO")} />
        );
      case "CHECKBOX":
        return (
            <Checkbox options={checkboxOptions} name={name} selectedValue={selectedCheckbox} onChange={(e)=>handleChange(e.target.value, "CHECKBOX")} />
        );
      default:
        return (
          <input
